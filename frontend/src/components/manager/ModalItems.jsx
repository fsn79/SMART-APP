...
function ModalItems({ el }) {
  ...
  return (
    <div className={el.status ? 'wc-item' : 'wc-item close'} key={el.id}>
      ...
      <div className='listTable' title={el.name}>{el.name}</div>
      <div className='listTable' title={el.partnumber}>{el.partnumber}</div>
      <div className='listTable' title={el.description}>{el.description}</div>
